...
import {
  APIresetUnseenMessageCount,
  getChatBoxUserList,
  setLastActiveParticipant,
...
export default function UserList({
  ...
  const handleContinue = async (selectedEmail: string) => {
    const selectedUser = users.find((user) => user.email === selectedEmail);
    if (selectedUser) {
      dispatch(setChatBoxActiveUser(selectedUser));
      onOpenChange(false);
      onContinueToChatBox();
      await setLastActiveParticipant(selectedUser._id);
      const resData = await APIresetUnseenMessageCount(selectedUser._id);
      if (resData.success) {
